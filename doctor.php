...
$sql_comand_name = "SELECT *FROM doctor_info;";
...
if(isset($_POST['submit-form'])){
    ...
    $add_mysql = "INSERT INTO doctor_info VALUES(0, '{$name}','{$surname}','{$father}', '{$nomer}');";
  ...
  <body>
      <div id="windowadd">    
          <div class="info1">
            ...
            <div>
            <button  name = "submit-form" type = "submit"  class = "btn btn-dark">Добавить</button>
</form>
</div>
        </div >
      </div>
...
while($row = mysqli_fetch_array($result_name)){
  ...
  $nomer_array[$index] = $row['nomer'];

  $name_doctor = $name_array[$index];   
  $surname_doctor = $surname_array[$index];
  $father_doctor = $father_array[$index]; 

  $nomer_doctor = $nomer_array[$index];
  $nomer = $nomer_doctor;
      ...
      <form>
        ...
        <h3><a href ="work.php?link='.$nomer.'">'.$name_doctor.' '.$surname_doctor.'</a></h3>
        <h2>№ отделения:'.$nomer_doctor.'</h2>
        <h6>'.$father_doctor.'</h6>
        </form>
      </div>';
